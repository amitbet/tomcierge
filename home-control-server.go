...
func (s *HomeControlServer) InitMqtt() {
	...
	s.MqttSub("cmnd/" + s.Configuration.MachineName)
	//s.SendMqttStatisticsInterval()
}
...
// func (s *HomeControlServer) SendMqttStatisticsInterval() {
...
func (s *HomeControlServer) SendMqttStatistics() {
	dt := time.Now()
	dt.UnixMilli()
	dur := (time.Now().Sub(s.StartTime))

	durTime := Timespan(dur)

	//.Format("T15:04:05")) // 02:09:37
	mStats := MachineStats{
